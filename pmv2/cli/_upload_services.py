...
from pmv2.logic import upload_services as logic
from pmv2.logic.upload_services import capacity_dict
...
def upload_file(  # pylint: disable=too-many-arguments
    ...
    try:
        inserted = asyncio.run(
            logic.upload_services(urban_client, gdf, service_type_id, physical_object_type_id, parallel_workers)
        )
    except KeyboardInterrupt:
        config.logger.error("Got interruption signal, impossible to save results")
        raise
    inserted = [s.model_dump() for s in inserted]
...
def upload_bulk(  # pylint: disable=too-many-arguments,too-many-locals
    ...
    for file in sorted(input_dir.glob("*.geojson")):
        ...
        try:
            ...
            raise
        results[file.name] = [s.model_dump() for s in inserted]
